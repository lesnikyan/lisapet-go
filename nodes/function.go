...
// 	Args    []*obj.ArgExp
// 	mtInst  *obj.ArgExp // obj instance for method
...
// func (fn *Function) InitArg(cx base.Context, ex base.Expression) (*obj.ArgExp, error) {
// 	// fmt.Printf(" Fu.InitArg#0  argExp=(%T, %v) \n", ex, ex)
// 	switch arx := ex.(type) {
// 	case *VarExpr:
// 		// rex := &ArgExp{Name: arx.name, VExp: arx}
// 		rex := &obj.ArgExp{Name: arx.name}
// 		return rex, nil
...
// 		rex := &obj.ArgExp{Name: name, Type: vtype, StrictType: strict}
...
// 		rex := &obj.ArgExp{Name: name, Type: vtype, StrictType: true}
...
// 	nArgs := make([]*obj.ArgExp, len(fn.dfnArgs))
...
func (fn *FuncBlock) Get() *base.Val {
	return fn.resVal
}

func (fn *FuncBlock) GetPopUp() *base.PopUp {
	return fn.Block.GetPopUp()
}

func (fn *FuncBlock) InitArg(cx base.Context, ex base.Expression) (*obj.ArgExp, error) {
	// fmt.Printf(" Fu.InitArg#0  argExp=(%T, %v) \n", ex, ex)
	switch arx := ex.(type) {
	case *VarExpr:
		rex := &obj.ArgExp{Name: arx.name}
...
func (fn *FuncBlock) Init(cx base.Context) error {
	// fmt.Printf(" Fu.Init  deArgLen==%d  \n", len(fn.dfnArgs))
	fn.defVals = make(map[string]*base.Val)
	nArgs := make([]*obj.ArgExp, len(fn.dfnArgs))
	for i, ex := range fn.dfnArgs {
...
func (fn *FuncBlock) SetArgVals(vals []any, nvals map[string]any) {
	// fmt.Printf(" Fu.SetArgVals  ovs=%d, nvs:%d  \n", len(vals), len(nvals))
	fn.argVals = vals
	fn.nmVals = nvals
}

func (fn *FuncBlock) getVal(i int, name string) (any, error) {
...
// func (fn *FuncBlock) Do(cx base.Context) error {
// 	return fn.Block.Do(cx)
// }

func (fn *FuncBlock) Do(inCx base.Context) error {
	// fn.res = nil
	fn.resVal = nil
